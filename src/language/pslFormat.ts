...
function buildText(method: parser.Method): string {
	...
	const parameterStrings: Param[] = method.parameters.map(p => {
		const param = { parameter: "", comment: "" };
		let parameterString = "";
		if (p.req) {
			parameterString += "req ";
		}
