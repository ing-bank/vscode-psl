...
export function diagnosticsOnLine(lineNumber: number, diagnostics: api.Diagnostic[]): api.Diagnostic[] {
	let diagnosticsOnLine = diagnostics.filter(r => r.range.start.line === lineNumber);
...
export async function getDiagnostics(testFileName: string, ruleName?: string): Promise<api.Diagnostic[]> {
	...
	let pslDocument = new api.PslDocument(api.parseText(text), text, testFilePath);
	let diagnostics = activate.getDiagnostics(pslDocument, false);
